...
class MarkerProcessor:
    ...
    def clear_marker_memory(self):
        """
        Полностью освобождает память моделей marker
        """

        if hasattr(self, "model_lst") and self.model_lst is not None:
            for model in self.model_lst.values():
                del model
    ...
    def processing_chunks(self):
        ...
        for block in tqdm(self.rendered_text.blocks, f"Преобразование чанков"):
            ...
            # Оборачиваем чанки с кодом и уравнениями в баксы, так как они в формате LaTex
